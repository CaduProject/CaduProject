...
async function call_cadu_tag_by_anime_api(
  discord_id: string | undefined,
  tag: string | undefined,
  anime_names: string | undefined
) {
...
export async function tag_by_anime(
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `Meu parceiros aqui estão os comandos que você vai precisar para tagear suas cartinhas:\n\nObs.: Por limitação da karuta cada conjunto possui 20 cartas para serem tageadas`
    )
    .setFields(fields)
    .setTimestamp();
